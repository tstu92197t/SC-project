...
def search_names(name_data, target):
    ...
    for key in name_data:
        ...
        for i in range(len(key) - len(target) + 1):  # the comparison between key and target has len(ref)-len(read)+1 times
            for j in range(len(target)):
                if key[j + i] == target[j]:
                    similarity += 1
            if similarity == len(target):
                lst += [key_original]  # since key is the lower case, and we want the original key; therefore, key_original
            similarity = 0
    return lst
